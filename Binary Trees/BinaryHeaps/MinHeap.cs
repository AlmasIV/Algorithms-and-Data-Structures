...
public class MinHeap<T> : IEnumerable<T> where T : IComparable<T>
{
	...
	private void HeapifyDown(int index)
	{
		...
		while (true)
		{
			...
			if (left < Length && _items[smallestIndex].CompareTo(_items[left]) > 0)
			{
				smallestIndex = left;
			}
			if (right < Length && _items[smallestIndex].CompareTo(_items[right]) > 0)
			{
				smallestIndex = right;
			}
			...
			T temp = _items[smallestIndex];
			_items[smallestIndex] = _items[index];
			_items[index] = temp;
			index = smallestIndex;
		}
	}
	...
	public static bool IsMinHeap(T[] binaryTree)
	{
